...
class PkHtmlPainter extends PkHtmlRenderer {
  ...
  public function mkDelBtn($args = []) {
    ...
    $ps_key = keyVal('ps_key', $method_vars);
    $hr = new PkHtmlRenderer();
    return $this->delete_button = $this->injectTpl($hr->$tag($content, keyVal('attributes', $res)), $ps_tpl, $ps_key);
  }
...
//    $errors = $errorMsgBag->get('error');
   ...
   * THIS IS PRETTY BAD - REFACTOR OTHER CODE & REPLACE THIS METHOD
  ...
  /** Makes an HTML table from a set of rows and optional table attributes
   * 
   * @param stringish|PartialSet|Array $rowsdata - all the row data of the table 
   *   Can either be already stringified, or it can be an array of arrays or 
   *   an array of arrays of arrays, which are incemented through & fed to $this->mkTblRow()
   *   Example: $rowdatas could be:

      [
          [['Years Experience:', $bouncer->yrsexp],$td_classes],
          [['Height:', $bouncer->getHeight()],$td_classes],
          [['Weight:', $bouncer->weight.' lbs'],$td_classes],
          [['Years Experience:', $bouncer->yrsexp],$td_classes],
      ]

   * 
   * @param string|assoc array $tblatts - if string, classname of the table, if
   *   assoc array, the attributes.
   */
  public function mkHtmlTbl($rowdatas,$tblatts=null) {
    if (is_array($rowdatas)) {
      ...
      foreach ($rowdatas as $rowdata) {
        if (is_array($rowdata[0])) {
          ...
          $rowatts = keyVal(2,$rowdata);
          $rows[] = $this->mkTblRow($tds,$tdatts,$rowatts);
        } else {
          $rows[] = $this->mkTblRow($rowdata);
        }
      }
   ...
   * @param indexed_array $tdatts - optional - should be the same size as $tds -
   *   the individual atts are either strings (td css classes) or an associative array
   *   of attributes, to apply to that td with the same integer index
   * @param string $rowatts - optional - if a string, the CSS class of the row, otherwise
   * an associative array of atts to apply to the row
   */
